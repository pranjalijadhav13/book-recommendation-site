...
if (strlen($q)>0) {
  ...
  for($i=0; $i<($x->length); $i++) {
    echo $x->length;
    if(strcmp($w,"Book Name") == 0) {
      echo "HI";
      $y=$x->item($i)->getElementsByTagName("book_name");
    }
    else if($w == "Author") $y=$x->item($i)->getElementsByTagName("author");
    else if($w == "Genre") $y=$x->item($i)->getElementsByTagName("genre");
   else if($w == "Age Group") $y=$x->item($i)->getElementsByTagName("agegroup");
